...
class ParseBED(object):
	...
	def getExon(self,outfile=None):
		...
		if outfile is None:outfile = self.fileName + ".exon.xls"
		FO=open(outfile,'w')
	...
	def getCDSExon(self,outfile=None):
	...
	def getBedinfor(self,outfile=None):
		...
		for line in self.f:
			...
			FO.write(str(exon_num) + "\t" + str(sum(exon_sizes)) +'\t' +str(min(exon_sizes)) + '\t' + str(max(exon_sizes)) +'\t' + str(old_div(sum(exon_sizes),exon_num)) +"\t")
			if intron_sizes:
				FO.write(str(sum(intron_sizes)) +'\t' + str(min(intron_sizes)) + '\t' + str(max(intron_sizes)) +'\t' + str(old_div(sum(intron_sizes),len(intron_sizes))) +"\n")		
			else:
				FO.write('0' +'\t' + '0' + '\t' + '0' +'\t' + '0' +"\n")	
...
class CompareBED(object):
	...
	def distribBed(self,outfile=None):
		...
		for line in self.A_fh:
			...
			if(len(fields)==12 and fields[9]>1):
				...
				mid = int(fields[1]) + int(old_div((int(fields[2])-int(fields[1])),2))
	...
	def findClosestTSS(self,outfile=None,downStream=50000,upStream=50000):
		...
		for line in self.A_fh:
			...
			chrom = fields[0]
			bed_st =  int(fields[1]) + int(old_div((int(fields[2]) - int(fields[1])),2))
			bed_end = bed_st +1
